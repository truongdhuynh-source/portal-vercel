...
export function Toolbar({ toolbars, disabled, ...props }) {
  ...
  return (
    <div className="oda-toolbar">
      {viewer && (
        <React.Fragment>
          {Object.keys(toolbars)
            .map((group) => toolbars[group])
            .filter((group) => when(group.visible))
            .map((group, index) => (
              <ToolbarGroup
                key={index}
                disabled={disabled || !databaseLoaded}
                items={group.items}
